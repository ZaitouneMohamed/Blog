                        ...
                        <div class="category-item">
                            <div class="category-img">
                                <a href="{{ route('post.show', $item) }}"><img
                                        src="{{ asset('assets/posts') }}/{{ $item->image }}" alt=""
                                        class="img-fluid w-100"></a>
                            </div>
                            <div class="content">
                                <a href="#"
                                    class="text-color text-uppercase font-sm letter-spacing font-extra">{{ $item->categorie->name }}</a>
                                <h4><a href="{{ route('post.show', $item) }}">{{ $item->created_at->format('F d , Y ') }}</a>
                                </h4>
                            </div>
                        </div>
                    </div>
            ...
            <div class="row">
                ...
                <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                    <div class="row">
                        <div class="col-lg-6 col-md-6 col-sm-6">
                            @foreach (\App\Models\Posts::orderBy('id', 'desc')->where('published', 1)->get()->take(4) as $item)
                                <article class="post-grid mb-5">
                                    ...
                                    <span
                                        class="letter-spacing cat-name font-extra text-uppercase font-sm text-color ">{{ $item->categorie->name }}</span>
                                    <h3 class="post-title mt-1"><a
                                            href="{{ route('post.show', $item) }}">{{ $item->title }}</a></h3>
                                    <span
                                        class="text-muted letter-spacing text-uppercase font-sm">{{ $item->created_at->format('F d , Y ') }}</span>
                                    <div class="post-content mt-4">
                                        <p>
                                            {{ Str::limit($item->title, 50, '...') }}
                                        </p>
                                        <a class="btn btn-grey mt-3" href="{{ route('post.show', $item) }}">read more</a>
                                    </div>
                                </article>
                            @endforeach
                        </div>
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-6">
                            @foreach (\App\Models\Posts::orderBy('views','desc')->where('published', 1)->get()->take(3) as $item)
                                <article class="post-grid mb-5">
                                    <a class="post-thumb mb-4 d-block" href="{{ route('post.show', $item) }}">
                                        <img src="{{ asset('assets/posts') }}/{{ $item->image }}" alt=""
                                            class="img-fluid w-100">
                    ...
                    <div class="pagination mt-5 pt-4">
                        <a class="btn btn-success mt-3" style="color: white" href="{{ route('posts_list') }}"> read
                            more</a>
                    </div>
                </div>
